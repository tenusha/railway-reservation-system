...
<code>export function getHash(str) {     </code>
<code> let hash = 0     </code>
<code> for (let i = 0; i < str.length; i++) {        </code>
<code>  hash += Math.pow(str.charCodeAt(i) * 31, str.length - i)      </code>   
<code>  hash = hash & hash // Convert to 32bit integer   </code>  
<code> }     </code>
